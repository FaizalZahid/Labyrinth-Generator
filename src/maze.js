var Maze = function(doc, elemId) {
  this.canvas = doc.getElementById(elemId);
  this.width = this.canvas.width;
  this.height = this.canvas.height;
  this.ctx = this.canvas.getContext('2d');
  
  var self = this;

  self.ctx.strokeStyle = "rgb(255, 255, 255)";
  self.ctx.fillStyle = "rgb(0, 0, 0)";
  self.ctx.fillRect(0, 0, self.width, self.height);

  return {
    width: function() {
      return self.width;
    },

    height: function() {
      return self.height;
    },

    draw: function(graph) {
      self.cellWidth = self.width / graph.width;
      self.cellHeight = self.height / graph.height;

      this.drawBorders(graph);
      this.drawMaze(graph);
    },

    drawBorders: function(graph) {
      this.drawLine(self.cellWidth, 0, self.width, 0);
      this.drawLine(self.width, 0, self.width, self.height);
      this.drawLine(self.width - self.cellWidth, self.height, 0, self.height);
      this.drawLine(0, self.height, 0, 0);
    },

    drawSolution: function(path) {
      self.ctx.fillStyle = "rgba(255, 255, 255, 0.8)";

      for (var i = 0; i < path.length; i++) {
        (function() {
          var cell = path[i];
          var x = cell.x * self.cellWidth;
          var y = cell.y * self.cellHeight;

          setTimeout(function() {
            self.ctx.fillRect(x, y, self.cellWidth, self.cellHeight);
          }, 80 * i);
        })();
      }
    },

    drawMaze: function(graph) {
      var drawnEdges = [];

      var edgeAlreadyDrawn = function(cell1, cell2) {
        return drawnEdges.find(function(edge) {
          return edge.includes(cell1) && edge.includes(cell2);
        }) !== undefined;
      };

      for (var i = 0; i < graph.width; i++) {
        for (var j = 0; j < graph.height; j++) {
          var cell = graph.cells[i][j];
          var topCell = graph.fetchCellAt(cell.x, cell.y - 1);
          var leftCell = graph.fetchCellAt(cell.x - 1, cell.y);
          var rightCell = graph.fetchCellAt(cell.x + 1, cell.y);
          var bottomCell = graph.fetchCellAt(cell.x, cell.y + 1);

          if (
            !edgeAlreadyDrawn(cell, topCell) &&
            graph.cellsAreLinked(cell, topCell)
          ) {
            var x1 = cell.x * self.cellWidth;
            var y1 = cell.y * self.cellHeight;
            var x2 = x1 + self.cellWidth;
            var y2 = y1;

            this.drawLine(x1, y1, x2, y2);
            drawnEdges.push([cell, topCell]);
          }

          if (
            !edgeAlreadyDrawn(cell, leftCell) &&
            graph.cellsAreLinked(cell, leftCell)
          ) {
            var x1 = cell.x * self.cellWidth;
            var y1 = cell.y * self.cellHeight;
            var x2 = x1;
            var y2 = y1 + self.cellHeight;

            this.drawLine(x1, y1, x2, y2);
            drawnEdges.push([cell, leftCell]);
          }

          if (
            !edgeAlreadyDrawn(cell, rightCell) &&
            graph.cellsAreLinked(cell, rightCell)
          ) {
            var x1 = cell.x * self.cellWidth + self.cellWidth;
            var y1 = cell.y * self.cellHeight;
            var x2 = x1;
            var y2 = y1 + self.cellHeight;

            this.drawLine(x1, y1, x2, y2);
            drawnEdges.push([cell, rightCell]);
          }

          if (
            !edgeAlreadyDrawn(cell, bottomCell) &&
            graph.cellsAreLinked(cell, bottomCell)
          ) {
            var x1 = cell.x * self.cellWidth;
            var y1 = cell.y * self.cellHeight + self.cellHeight;
            var x2 = x1 + self.cellWidth;
            var y2 = y1;

            this.drawLine(x1, y1, x2, y2);
            drawnEdges.push([cell, bottomCell]);
          }
        }
      }
    },

    drawLine: function(x1, y1, x2, y2) {
      self.ctx.beginPath();
      self.ctx.moveTo(x1, y1);
      self.ctx.lineTo(x2, y2);
      self.ctx.stroke();
    }
  };
};
